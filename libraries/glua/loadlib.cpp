...
/**
 * module searcher from thinkyoung api
 */
static int searcher_thinkyoung(lua_State *L) {
    const char *name = luaL_checkstring(L, 1);
    if (!name)
        return LUA_ERRERR;
...
static void createsearcherstable(lua_State *L) {
    ...
    { searcher_preload, searcher_thinkyoung, searcher_C, searcher_Croot, nullptr }; // searcher_Lua 在searcher_preload后

    static const lua_CFunction thinkyoung_searchers[] =
    { searcher_preload, searcher_thinkyoung };
    ...
    /*
    lua_createtable(L, sizeof(thinkyoung_searchers) / sizeof(thinkyoung_searchers[0]) - 1, 0);

    for (i = 0; thinkyoung_searchers[i] != nullptr; i++) {
    lua_pushvalue(L, -2);
    lua_pushcclosure(L, thinkyoung_searchers[i], 1);
    lua_rawseti(L, -2, i + 1);
    }
    */
    // lua_setfield(L, -4, "thinkyoung_searchers");
}
