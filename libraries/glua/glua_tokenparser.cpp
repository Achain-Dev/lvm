        ...
        void GluaTokenParser::_read_long_string(std::string &code, bool is_string, int sep)
        {
            ...
            while (_parsing_pos < code.length())
            {
                if (_parsing_buff_len >= LUA_TOKEN_MAX_LENGTH - 1)
                {
					...
					{
                        global_glua_chain_api->throw_exception(_L, LVM_API_COMPILE_ERROR, std::string("too long token").c_str());
						return;
					}
                }
                switch (_current_char(code))
                {
                case EOF_TOKEN_CHAR:
                    global_glua_chain_api->throw_exception(_L, LVM_API_COMPILE_ERROR, "undefined long %s (starting at line %d)", what, line);
                    return;
                case ']':
                {
