...
void luaD_throw(lua_State *L, int errcode) {
    if (L->errorJmp) {  // thread has an error handler?
        ...
        if (g->mainthread->errorJmp) {  // main thread has a handler? 
            ...
            // abort();
            global_glua_chain_api->throw_exception(L, LVM_API_SIMPLE_ERROR, errmsg.c_str());
...
static void tryfuncTM(lua_State *L, StkId func) {
    ...
    if (!ttisfunction(tm))
    {
        global_glua_chain_api->throw_exception(L, LVM_API_LVM_ERROR, "Can't find __call method");
